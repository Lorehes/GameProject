...
title_font = pygame.font.SysFont("applegothic,applesangothicneo,arial", 80)
guide_font = pygame.font.SysFont("applegothic,applesangothicneo,arial", 20)
...
while not exit:  # 시작화면 → 게임 → 결과 → 시작화면 반복
    ...
    while not exit and not ready:
        ...
        title = title_font.render("행맨 게임", True, white)
        title_size = title.get_size()
        title_pos = (size[0]/2 - title_size[0]/2, size[1]/2 - title_size[1]/2)
        screen.blit(title, title_pos)

        guide = guide_font.render("Press any key to start", True, white)
        guide_size = guide.get_size()
        guide_pos = (size[0]/2 - guide_size[0]/2, size[1]*4/5 - guide_size[1]/2)
    ...
    while not exit:
        ...
        if game.is_finished():
            ...
            sub = guide_font.render("Press any key to restart", True, white)
            sub_pos = (size[0]/2 - sub.get_width()/2, size[1]/2 + msg_size[1])
            ...
            while waiting and not exit:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        waiting = False
            break  # 게임 루프 탈출 → 외부 루프에서 시작화면 재진입
        ...
        # 틀린 횟수에 따라 신체 부위 점진적으로 표시
        tries = game.try_num
        if tries >= 1:
            pygame.draw.circle(screen, white, head, r_head_point, 3)
        if tries >= 2:
            pygame.draw.line(screen, white, neck_top, neck_bottom, 3)
        if tries >= 3:
            pygame.draw.line(screen, white, body_top, body_bottom, 3)
        if tries >= 4:
            pygame.draw.line(screen, white, arm_origin, left_arm_end, 3)
        if tries >= 5:
            pygame.draw.line(screen, white, arm_origin, right_arm_end, 3)
        if tries >= 6:
            pygame.draw.line(screen, white, body_bottom, left_leg_end, 3)
        if tries >= 7:
            pygame.draw.line(screen, white, body_bottom, right_leg_end, 3)
        ...
        if history:
            ...
            for i, letter in enumerate(history):
                x = lx + i * (box_w + gap)
                pygame.draw.rect(screen, white, (x, ly, box_w, box_h))
                surf = letter_font.render(letter, True, black)
